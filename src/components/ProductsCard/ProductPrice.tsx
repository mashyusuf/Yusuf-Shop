...
function ProductPrice({product}:{product:ProductType}) {
  ...
  return (
    <div>
        <PriceFormate amount={existingProduct? discountedPrice * existingProduct?. quantity!:discountedPrice} className="text-gray-500 line-through font-normal" />
        <PriceFormate amount={existingProduct? regularPrice * existingProduct?. quantity!:regularPrice} className='font-semibold text-sky-600' />
    </div>
  )
}
